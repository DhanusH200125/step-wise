...
import {
  Bell, CheckCheck, Trash2, X, Clock, AlertCircle,
  Calendar, TrendingUp, Info
} from 'lucide-react';


// Visual styles and icons mapped to specific notification categories
const TYPE_CONFIG = {
  task_reminder: { icon: Clock, color: 'text-primary', bg: 'bg-primary10' },
  deadline_reminder: { icon: Clock, color: 'text-orange', bg: 'bg-orange10' },
  overdue: { icon: AlertCircle, color: 'text-notification', bg: 'bg-notification10' },
  planning_reminder: { icon: Calendar, color: 'text-gold', bg: 'bg-gold10' },
  retrospective_reminder: { icon: TrendingUp, color: 'text-success', bg: 'bg-success10' },
...
// Helper function to format timestamps into relative time strings (e.g., '5m ago')
...
export default function NotificationBell() {
  ...
  const fetchNotifs = useCallback(async () => {
    try {
      // Trigger a server-side check for new notifications based on current time
  ...
  useEffect(() => {
    fetchNotifs();
    pollRef.current = setInterval(fetchNotifs, 5 * 60 * 1000);
  ...
  const markRead = async (id) => {
    // Optimistically update local state before API confirmation
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        {unread > 0 && (
          <span className="absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] bg-notification text-white text-[10px] font-black rounded-full flex items-center justify-center px-1 leading-none">
            {unread > 9 ? '9+' : unread}
          </span>
        )}
      </button>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-start gap-3 px-4 py-3.5 text-left transition-all hover:bg-surface-offset
                      ${!notif.isRead ? 'bg-primary10' : ''}`}
                  >
                    <div className={`w-8 h-8 rounded-xl ${cfg.bg} flex items-center justify-center flex-shrink-0 mt-0.5`}>
                      <Icon className={`w-3.5 h-3.5 ${cfg.color}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm leading-snug ${!notif.isRead ? 'font-bold text-text' : 'text-text-muted'}`}>
                        {notif.message}
                      </p>
